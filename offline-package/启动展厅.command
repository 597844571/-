...
if [ "$ARCH" = "arm64" ] && [ -x "runtime/展厅服务-mac-arm64" ]; then
    echo "=========================================="
    echo "  正在启动神枢数字展厅（Apple Silicon）..."
    echo "=========================================="
    "runtime/展厅服务-mac-arm64" &
    SERVER_PID=$!
elif [ "$ARCH" = "x86_64" ] && [ -x "runtime/展厅服务-mac-x64" ]; then
    echo "=========================================="
    echo "  正在启动神枢数字展厅（Intel Mac）..."
    echo "=========================================="
    "runtime/展厅服务-mac-x64" &
    SERVER_PID=$!
else
    ...
    if [ -n "$NODE_CMD" ]; then
        ...
        if [ -n "$PYTHON_CMD" ]; then
            ...
            echo "=========================================="
            cd dist && "$PYTHON_CMD" -m http.server $PORT &
            SERVER_PID=$!
            cd - > /dev/null
        else
