...
/* ============================================================
   V3.2 合伙权益增长沙盘 — 自适应一屏 · 标签角度分布 · 金额截断
   ============================================================ */
  ...
  return (
    <div className="relative h-full flex flex-col items-center overflow-hidden" onWheel={handleWheel}>
      ...
      <button onClick={handlePrev} disabled={currentIdx === 0}
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.textMuted} strokeWidth="2" strokeLinecap="round"><polyline points="18 15 12 9 6 15" /></svg>
      </button>
      ...
      <div className="relative flex-1 w-full" style={{ perspective: '600px' }}>
        <div className="absolute inset-0 flex flex-col items-center"
          ...
          {IDENTITY_ORDER.map((id, idx) => {
            ...
            const opacity = isActive ? 1 : dist === 1 ? 0.5 : dist === 2 ? 0.18 : 0.06
            const scale = isActive ? 1 : dist === 1 ? 0.88 : dist === 2 ? 0.74 : 0.6
            ...
            return (
              <button key={id} onClick={() => onSelect(id)} className="w-full text-left flex-shrink-0 transition-all"
                ...
                <div className="mx-1 rounded-lg px-2.5 py-1.5 h-full flex flex-col justify-center transition-colors"
                  style={{
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs font-medium" style={{ color: isActive ? C.textMain : C.textSub }}>{label}</span>
                    {isActive && (
      ...
      <button onClick={handleNext} disabled={currentIdx === IDENTITY_ORDER.length - 1}
        ...
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={C.textMuted} strokeWidth="2" strokeLinecap="round"><polyline points="6 9 12 15 18 9" /></svg>
      </button>
    </div>
  )
}
...
function EquityStarCore({ result, playingStep }: { result: GrowthResult; playingStep: number }) {
  ...
  // 标签按 12点/3点/6点/9点 分布，避免重叠
  const rings = [
    { label: '产品礼包', value: result.productGiftValue, color: C.dataBlue, angle: -90 },     // 上方
    { label: '数据资产', value: result.dataAssetReward, color: C.flowCyan, angle: 0 },        // 右方
    { label: '可流通资产', value: result.tradableAsset, color: C.equityGold, angle: 90 },     // 下方
    { label: '每日释放', value: result.dailyReleaseMax, color: C.poolPurple, angle: 180 },    // 左方
  ]

  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {/* 最外层：分成光路 */}
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: ringsVisible ? 1 : 0.12 }} transition={{ duration: 0.8 }}
      ...
      {rings.map((ring, i) => {
        const size = 72 + i * 14
        const delay = ringsVisible ? i * 0.1 : 0
        const spinDir = i % 2 === 0 ? '' : 'reverse'
        const spinDur = 9 + i * 3
        // 标签位置计算
        const rad = (ring.angle * Math.PI) / 180
        const labelR = 50 // 标签放在圆环外侧
        const lx = 50 + Math.cos(rad) * labelR
        const ly = 50 + Math.sin(rad) * labelR

        return (
          <motion.div key={ring.label} initial={{ opacity: 0, scale: 0.85 }} animate={{ opacity: ringsVisible ? 1 : 0.1, scale: ringsVisible ? 1 : 0.9 }} transition={{ delay, duration: 0.45 }}
            ...
            {/* 角度分布标签 */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: ringsVisible ? 1 : 0 }} transition={{ delay: delay + 0.12 }}
              className="absolute flex flex-col items-center px-1.5 py-px rounded-md pointer-events-none"
              style={{
                background: `${ring.color}08`,
      ...
      {/* 中心星核 */}
      <motion.div initial={{ opacity: 0, scale: 0.4 }} animate={{ opacity: centerVisible ? 1 : 0.2, scale: centerVisible ? 1 : 0.7 }} transition={{ duration: 0.5, ease: 'easeOut' }}
        className="relative flex flex-col items-center justify-center z-10"
        style={{
          width: 'clamp(64px, 20%, 100px)',
          height: 'clamp(64px, 20%, 100px)',
          borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(74,184,255,0.18) 0%, rgba(124,92,255,0.1) 100%)',
          border: '1.5px solid rgba(74, 184, 255, 0.3)',
          boxShadow: '0 0 24px rgba(74,184,255,0.12), inset 0 1px 0 rgba(255,255,255,0.06)',
        }}>
  ...
  return (
    <div className="relative w-full h-full">
      <svg viewBox="0 0 300 240" className="w-full h-full">
        <defs>
          <radialGradient id="nwGlow2" cx="50%" cy="50%" r="50%">
            <stop offset="0%" stopColor="rgba(74,184,255,0.05)" />
            <stop offset="100%" stopColor="transparent" />
          </radialGradient>
        </defs>
        <circle cx={cx} cy={cy} r="130" fill="url(#nwGlow2)" />
        {nodes.map((n, i) => (
          <motion.line key={`l-${i}`} initial={{ opacity: 0 }} animate={{ opacity: visible ? 0.08 : 0 }} transition={{ delay: i * 0.02 }}
            x1={cx} y1={cy} x2={n.x} y2={n.y} stroke={n.color} strokeWidth="0.5" />
        ))}
        {nodes.map((n, i) => nodes.slice(i + 1).map((m, j) => {
          const dist = Math.hypot(n.x - m.x, n.y - m.y)
          if (dist > 65) return null
          return <motion.line key={`c-${i}-${j}`} initial={{ opacity: 0 }} animate={{ opacity: visible ? 0.04 : 0 }} transition={{ delay: 0.2 + i * 0.012 }}
            x1={n.x} y1={n.y} x2={m.x} y2={m.y} stroke={C.dataBlue} strokeWidth="0.4" />
        }))}
        <motion.circle cx={cx} cy={cy} r="130" fill="none" stroke="rgba(74,184,255,0.04)" strokeWidth="1" strokeDasharray="3 6"
    ...
    <div className="relative w-full h-full flex flex-col items-center justify-center">
      <div className="relative w-[60%] h-[60%] flex items-center justify-center">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: visible ? 1 : 0.2, scale: visible ? 1 : 0.7 }} transition={{ duration: 0.5 }}
          ...
          style={{
            background: 'radial-gradient(circle, rgba(124,92,255,0.2) 0%, rgba(74,184,255,0.12) 100%)',
        ...
        {streams.map((s) => (
          <motion.div key={s.label} initial={{ opacity: 0 }} animate={{ opacity: visible ? 1 : 0 }} className="flex flex-col items-center">
            <div className="flex items-center gap-1">
              ...
              <span className="text-[8px]" style={{ color: C.textMuted }}>{s.label}</span>
            </div>
...
function RevenueDashboard({ result, scenario, playingStep }: { result: GrowthResult; scenario: PartnerGrowthScenario; playingStep: number }) {
  ...
  return (
    <div className="flex flex-col gap-1.5 h-full">
      {/* 顶部大卡 */}
      <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: resultVisible ? 1 : 0.3, y: resultVisible ? 0 : -3 }} transition={{ duration: 0.4 }}
        ...
        <div className="flex items-center gap-3 mt-1.5">
          <div>
            ...
            <div className="text-xs font-bold" style={{ color: C.flowCyan }}>{fmtRoi(roi)}</div>
          </div>
          <div className="w-px h-4" style={{ background: 'rgba(126,190,255,0.06)' }} />
          <div>
            <span className="text-[9px]" style={{ color: C.textMuted }}>期末月度分成</span>
            <div className="text-xs font-bold" style={{ color: C.dataBlue }}>¥{fmtMoney(result.monthlyTrack[result.monthlyTrack.length - 1]?.equityShare ?? 0)}</div>
          </div>
        </div>
      </motion.div>
...
function BattleTrack({ track, months, playingStep, playbackMonth }: { track: MonthlyData[]; months: number; playingStep: number; playbackMonth: number }) {
  ...
  return (
    <div className="w-full overflow-x-auto scrollbar-thin">
      <svg viewBox={`0 0 ${w} ${h + 56}`} className="w-full" style={{ minWidth: '600px' }}>
        <defs>
          <linearGradient id="btGrad2" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor={C.dataBlue} stopOpacity="0.35" />
            <stop offset="100%" stopColor={C.poolPurple} stopOpacity="0.35" />
          </linearGradient>
          <linearGradient id="btArea2" x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        {/* 淡色折线背景 */}
        <polyline points={points} fill="none" stroke="rgba(74, 184, 255, 0.05)" strokeWidth="1" strokeLinecap="round" />

        {/* 活跃折线 */}
        {activeMonths > 1 && (
          <polyline points={data.slice(0, activeMonths).map((d, i) => `${getX(i)},${getY(d.equityShare)}`).join(' ')} fill="none" stroke="url(#btGrad2)" strokeWidth="1.5" strokeLinecap="round" />
        )}
        ...
        {keyIndices.map((idx) => {
          ...
          return (
            <g key={`k-${idx}`}>
              <line x1={x} y1={y} x2={x} y2={h + 2} stroke={isActive ? `${C.dataBlue}15` : 'rgba(126,190,255,0.03)'} strokeWidth="1" strokeDasharray="2 2" />
              ...
              {isCurrent && (
                <circle cx={x} cy={y} r="6" fill="none" stroke={C.dataBlue} strokeWidth="0.8" opacity="0.35">
              ...
              <g transform={`translate(${x}, ${h + 6})`}>
                <rect x="-40" y="0" width="80" height="46" rx="5"
                  fill={isActive ? 'rgba(11, 20, 36, 0.65)' : 'rgba(11, 20, 36, 0.35)'}
                  stroke={isActive ? 'rgba(74, 184, 255, 0.1)' : 'rgba(126, 190, 255, 0.04)'} strokeWidth="1" />
                <text x="0" y="12" textAnchor="middle" fill={isActive ? C.equityGold : 'rgba(246,201,107,0.2)'} fontSize="10" fontWeight="bold">M{d.month}</text>
                <text x="0" y="26" textAnchor="middle" fill={isActive ? C.textSub : 'rgba(176,196,232,0.2)'} fontSize="8">+{d.newCustomers}人 ¥{fmtMoney(d.cumulativeInvestment)}</text>
                <text x="0" y="40" textAnchor="middle" fill={isActive ? C.dataBlue : 'rgba(74,184,255,0.15)'} fontSize="9" fontWeight="bold">¥{fmtMoney(d.equityShare)}</text>
              </g>
            </g>
          )
        })}
...
export default function PartnerGrowthSandbox() {
  ...
  // 精确一屏高度：减去顶部导航(72px) + 底部状态栏(~28px)
  return (
    <div className="relative w-full flex flex-col" style={{ height: 'calc(100vh - 72px - 28px)' }}>
      {/* 标题栏 */}
      <div className="flex items-center justify-between px-4 h-9 flex-shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-[9px]" style={{ color: C.textMuted }}>神枢算力权益通 · 策略推演</span>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      {/* 推演假设 */}
      <div className="px-4 h-6 flex-shrink-0 flex items-center gap-2">
      ...
      <div className="px-4 h-6 flex-shrink-0 flex items-center">
        <AssumptionBadge scenario={scenario} />
      </div>

      {/* 主内容区 — 精确一屏 */}
      <div className="flex-1 grid grid-cols-12 gap-2 min-h-0 px-4 py-1.5">
        {/* 左侧：身份轨道 */}
      ...
      {/* 底部：增长轨道 */}
      <div className="h-[80px] flex-shrink-0 mx-4 mb-2 rounded-xl p-2 relative" style={{ background: C.panelBg, border: `1px solid ${C.panelBorder}` }}>
